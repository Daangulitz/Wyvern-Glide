...
namespace YawVR
{
    ...
    public class MotionCompensation : MonoBehaviour
    {
        ...
        {
            // Fixed during the SOLID refactor: this was `!= Started || != Connected`,
            // which is a tautology (always true for any single state value) — LateUpdate
            // always returned early, so camera compensation never actually ran. The
            // evident intent, restored here, is "skip unless we're Started or Connected".
